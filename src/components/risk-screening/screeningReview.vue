...
import basicList from '@/components/risk-screening/screening-review-basic/list'
import productList from '@/components/risk-screening/screening-review-product/list'
export default {
  name: 'screeningReview',
  data () {
    return {
      breadcrumb: ['隐患排查治理', '排查复核'],
      type: '基础类排查清单', // tab切换类型
      isChildUpdate1: true, // tab1切换开关，刷新
      isChildUpdate2: false, // tab2切换开关，刷新
      hiddInstanceId: '' // 路由 businessKey 的值
    }
  },
  components: {
    BreadCrumb, // 面包屑
  ...
  methods: {
    ...
    extractKey () {
      if (this.$route.query.hiddInstanceId) {
        ...
        // console.log(this.hiddInstanceId)
      }
    }
  }
}

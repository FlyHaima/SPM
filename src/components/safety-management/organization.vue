              ...
              <div class="container-box">
                <p class="btn-p"> <!-- 权限显示 -->
                  <a class="export-btn function-btn export-btn" target="_blank"
                     @click="exportLeader(triggerLeaderId, uploadData.token)"
                     v-if="fucBtns.includes('export-btn')"><i class></i>导出</a>
        ...
        <el-tab-pane name="tab_c">
          ...
          <el-container class="inner-main-content">
            ...
            <el-main class="inner-content">
              <div class="container-box">
                <p class="btn-p"> <!-- 权限显示 -->
                  <a class="function-btn export-btn" target="_blank"
                     @click="exportWorker(triggerWorkId, uploadData.token)"
                     v-if="fucBtns.includes('export-btn')"><i class></i>导出</a>
                  <el-upload accept=".xls" class="function-btn import-btn"
                            v-if="fucBtns.includes('import-btn')"
...
export default {
  ...
  methods: {
    ...
    exportLeader (deptId, token) {
      const getUrl = `${this.baseUrl}/leadUser/exportGroup?token=${token}&deptId=${deptId}`
      window.open(getUrl)
    },
    exportWorker (deptId, token) {
      const getUrl = `${this.baseUrl}/workUser/exportGroup?token=${token}&deptId=${deptId}`
      window.open(getUrl)
    }
  },
  components: {
